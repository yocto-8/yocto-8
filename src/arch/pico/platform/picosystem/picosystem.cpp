...
void init_default_frequency() {
	...
	// This may, however, be influenced by PCB design, so practical limits may
	// be different between the Pico and the PicoSystem.
...
void init_emulator() { emu::emulator.init(std::span<std::byte, 0>()); }
...
void init_video_st7789() {
	...
	picosystem::hw.st7789.init({.spi = video_spi,
	                            .pinout = {.sclk = 6,
	                                       .tx = 7,
	                                       .cs = 5,
	                                       .vsync = 8,
